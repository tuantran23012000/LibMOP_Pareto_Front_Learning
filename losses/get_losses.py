...
class LOG_func():
    def __init__(self,cfg):
        self.cfg = cfg
    def get_loss(self,pf,ray):
        F = torch.sum(ray*torch.log(pf+1))
        return F
...
class PROD_func():
    def __init__(self,cfg):
        self.cfg = cfg
    def get_loss(self,pf,ray):
        F = torch.prod((pf+1)**ray)
        return F
    
class UTILITY_func():
    def __init__(self,cfg):
        self.cfg = cfg
    def get_loss(self,pf,ray,ub):
        F = 1/torch.prod((ub - pf)**ray)
        return F
...
class Cheby_func():
    def __init__(self,cfg):
        self.cfg = cfg
    def get_loss(self,pf,ray,lower_bound):
        if lower_bound is not None:
            F = torch.max(ray*(pf-lower_bound))
        else:
            F = torch.max(ray*(pf))
        return F
    def get_target(self,pf,ray,lower_bound):
        if lower_bound is not None:
            F,_ = torch.max(ray*(pf-lower_bound),1)
        else:
            F ,_= torch.max(ray*(pf),1)
        return pf[F.argmin(), :]
class SmoothCheby_func():
    def __init__(self,cfg):
        self.cfg = cfg
    def get_loss(self,pf,ray):
        mu = 0.1
        F = mu*torch.log(torch.sum(torch.exp(ray*(pf)/mu)))
        return F
    def get_target(self,pf,ray):
        mu = 0.1
        F = mu*torch.log(torch.sum(torch.exp(ray*(pf)/mu),axis=1))
        return pf[F.argmin(), :]
...
class EPO_func():
    ...
    def get_loss(self, losses, ray, parameters=None, **kwargs):
        assert parameters is not None
        return self.solver.get_weighted_loss(losses, ray, parameters)
    def get_target(self,pf, ray, parameters=None, **kwargs):
        F = self.solver.get_weighted_loss(pf, ray, parameters)
        return pf[F.argmin(), :]
